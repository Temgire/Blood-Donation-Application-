package com.example.blooddonenationapp.Adapter;
...
import com.example.blooddonenationapp.Email.JavaMailApi;
import com.example.blooddonenationapp.Model.User;
import com.example.blooddonenationapp.R;
...
public class UserAdapter extends RecyclerView.Adapter<UserAdapter.ViewHolder> {
    ...
    public ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(context).inflate(R.layout.user_displyed_layout,parent,false);
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        Glide.with(context).load(user.getProfilePicture()).into(holder.userProfileImage);

        final String nameOfTheReceiver = user.getName();
        final String idOfTeeReceiver  = user.getId();
                                ...
                                reference.addValueEventListener(new ValueEventListener() {
                                    ...
                                    public void onDataChange(@NonNull DataSnapshot snapshot) {
                                        ...
                                        String mMessage = "Hello "+nameOfTheReceiver+" ,"+nameOfSender+
                                                ...
                                                "BLOOD DONATION APPLICATION - DONATE BLOOD , SAVE LIVES...";
                                        ...
                                        senderref.child(idOfTeeReceiver).setValue(true).addOnCompleteListener(new OnCompleteListener<Void>() {
                                            @Override
                                            public void onComplete(@NonNull Task<Void> task) {
                                                if (task.isSuccessful()){
                                                    DatabaseReference receiverref = FirebaseDatabase.getInstance().getReference("emails")
                                                            .child(idOfTeeReceiver);
                                                    receiverref.child(FirebaseAuth.getInstance().getCurrentUser().getUid()).setValue(true);

                                                    addNotifications(idOfTeeReceiver, FirebaseAuth.getInstance().getCurrentUser().getUid());
